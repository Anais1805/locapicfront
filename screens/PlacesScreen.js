...
export default function PlacesScreen() {
  ...
  const deleteClick = (PressedCity) => {
    console.log('city', PressedCity);
    fetch(`${url}/places`, {
      method: 'DELETE',
      headers: {
        'Accept': 'application/json',
  ...
  dispatch(removePlace(PressedCity))
}
  ...
  const places = user.places.map((data, i) => {
    return (
      <View key={i} style={styles.card}>
        ...
        <FontAwesome name='trash-o' onPress={() => deleteClick(data.name)} size={25} color='#ec6e5b' />
      </View>
    );
  });
